...
{
	static const int v25 =
		1365; // see https://github.com/ve3wwg/stm32f103c8t6/issues/11
	int vtemp;

	vtemp = (int)read_adc(ADC_CHANNEL_TEMP) * 3300 / 4095;

	return (v25 - vtemp) * 1000 / 45 +
	       2500; // temp = (1.43 - Vtemp) / 4.5 + 25.00
}
...
static void demo_task(void *arg __attribute((unused)))
{
	...
	for (;;) {
		...
		std_printf(
			"Temperature %d.%02d C, Vref %d.%02d Volts, ch0 %d.%02d V, ch1 %d.%02d V\n",
			temp100 / 100, temp100 % 100, vref / 100, vref % 100,
			adc0 / 100, adc0 % 100, adc1 / 100, adc1 % 100);
	}
}

static void adc_setup(void)
{
	/*	ADC Read on ADC1, Pin A1	*/
	...
	adc_power_on(ADC1); /* Wait for ADC starting up. */
	int i;
...
int main(void)
{
	/* Setup Data Structures, long queue for ADC, short queue for Filtering, One value for UART */

	/* Setup UART */

	/* Setup ADC with ISR on conversion completion */
	// Delete later rcc_peripheral_enable_clock(&RCC_APB2ENR,RCC_APB2ENR_ADC1EN);
	rcc_periph_clock_enable(RCC_ADC1);
